...
const ConfirmActionItems: React.FunctionComponent = () => {
  ...
  const MAP_BOX_ACCESS_TOKEN = "pk.eyJ1IjoiZGhlZXJhajEwMTkiLCJhIjoiY20zNDc3cmpyMWxibjJqcXNlZWc3YnhmciJ9.HpdXlHgrtmWfOtsVQ6oBHA";

  console.log(pickUpCoordinate, dropOffCoordinate);

  const getDropOffCoordinate = () => {
    const dropOffLocation = "london";

    fetch(
      `https://api.mapbox.com/geocoding/v5/mapbox.places/${dropOffLocation}.json?access_token=${MAP_BOX_ACCESS_TOKEN}`
    )
  ...
  const getPickUpCoordinate = () => {
    ...
    fetch(
      `https://api.mapbox.com/geocoding/v5/mapbox.places/${pickUpLocation}.json?access_token=${MAP_BOX_ACCESS_TOKEN}`
    )
      .then((response) => response.json())
      .then((data) => {
